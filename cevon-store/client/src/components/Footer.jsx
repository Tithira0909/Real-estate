  ...
  return (
    <footer className="bg-[#0B2529] text-white pt-20 pb-10">
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-gray-400">
              {['New Arrivals', 'Best Sellers', 'Rings', 'Necklaces', 'Earrings'].map(item => (
                <li key={item}>
                  <Link to={`/shop/${item === 'New Arrivals' || item === 'Best Sellers' ? 'All' : item}`} className="hover:text-white transition-colors">
                    {item}
                  </Link>
                </li>
              ))}
      ...
      >
        ...
        <span className="absolute right-full mr-4 bg-white text-[#0B2529] px-4 py-2 rounded-lg shadow-xl text-xs font-serif tracking-widest opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none hidden md:block">
            CHAT WITH US
        </span>
      </a>
    </footer>
