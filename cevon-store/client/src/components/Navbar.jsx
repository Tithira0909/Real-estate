...
const Navbar = () => {
  ...
  // Use glass nav if scrolled OR if not on the home page
  const navClass = isScrolled || location.pathname !== '/'
    ? 'glass-nav py-3'
    : 'bg-transparent py-6';

  const linkClass = location.pathname !== '/' ? 'text-[#0B2529]' : 'text-white';
  const logoClass = location.pathname !== '/' ? 'text-[#0B2529]' : 'text-[#D4AF37]';

  // Override for scrolled state (always dark text on glass except logo which might need adjustment)
  // Actually, glass nav usually sits on top of content. If it's dark glass, white text. If light glass, dark text.
  // The original design had a dark glass nav. Let's stick to that for consistency.
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 flex justify-between items-center text-white">
        ...
        {/* Logo */}
        <Link to="/" className={`text-4xl md:text-5xl font-signature tracking-wide cursor-pointer z-50 ${logoClass} drop-shadow-lg`}>
          Cevon
        </Link>
        ...
        <div className="hidden md:flex space-x-12 text-sm uppercase tracking-widest font-light">
          {['Collections', 'Atelier', 'Gifts', 'Journal'].map((item) => (
            <Link key={item} to={`/shop/${item.toLowerCase() === 'collections' ? 'All' : item}`} className="hover:text-[#D4AF37] transition-colors relative group">
              {item}
        ...
        <div className="flex items-center space-x-6">
          ...
          >
            ...
            {cartItemCount > 0 && (
              <span className="absolute -top-2 -right-2 w-4 h-4 bg-[#D4AF37] text-[#0B2529] rounded-full text-[10px] font-bold flex items-center justify-center">
                {cartItemCount}
              </span>
            )}
          </div>
    ...
    {mobileMenuOpen && (
      <div className="fixed inset-0 z-50 bg-[#0B2529] text-white flex flex-col justify-center items-center space-y-8 animate-fade-in">
        ...
        {['Collections', 'Atelier', 'Gifts', 'Journal', 'Admin'].map((item) => (
          <Link
            key={item}
            to={item === 'Admin' ? '/admin' : `/shop/${item.toLowerCase() === 'collections' ? 'All' : item}`}
